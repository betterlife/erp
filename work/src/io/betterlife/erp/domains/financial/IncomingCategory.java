...
@NamedQueries({
    ...
    @NamedQuery(name = "IncomingCategory.getAll",  query = "SELECT e FROM IncomingCategory e WHERE e.active = TRUE ORDER BY e.id DESC"),
    @NamedQuery(name = "IncomingCategory.getByKeyword",  query = "SELECT e FROM IncomingCategory e WHERE e.active = TRUE and (e.name like :keyword)")
})
